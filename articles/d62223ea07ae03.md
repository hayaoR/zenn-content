...
WebサーバーはGCがある言語(Python, Go, Java)で書かれることがほとんどですが、今後Rustで書かれることは増えていくのでしょうか？
...
## サンプルプログラム
サンプルプログラムを示します。ここで読み取って欲しいのはこのプログラムでは`Router`なるインスタンスを生成し、それを`Service`なるものに変換し、`Server`に渡しているということです。
